$(function () {
  ...
  $(".masonry-item").on("click", function () {
    ...
    $caption.html(
      "<b>" +
        title +
        "</b><br><span style='color:#42867b; font-size:13px;'>" +
        desc +
        "</span>",
    );
  ...
  $("#auditForm").on("submit", function (e) {
    ...
    $submitBtn
      .html(
        'Preparing Audit... <i class="fa-solid fa-spinner fa-spin" style="margin-left: 8px;"></i>',
      )
      .prop("disabled", true);
    ...
    $.ajax({
      ...
      error: function () {
        ...
        alert(
          "Oops! There was a problem processing your request. Please check your connection and try again.",
        );
        $submitBtn.html(originalText).prop("disabled", false);
      },
    });
  ...
  const GOOGLE_API_KEY = "AIzaSyC1cTi_QZrpiJdZhMCilYlxBGj4NvSvD-0";
    ...
    .addEventListener("submit", async function (e) {
      ...
      try {
        ...
        const response = await fetch(
          `https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url=${encodeURIComponent(urlInput)}&key=${GOOGLE_API_KEY}&strategy=mobile`,
        );
        ...
        scanBtn.innerHTML =
          'Scan Website <i class="fa-solid fa-magnifying-glass"></i>';
    ...
    .addEventListener("submit", function (e) {
      ...
      fetch("https://formspree.io/f/mnjbladd", {
        ...
        body: JSON.stringify({
          ...
          message:
            "New Website Audit Lead! They want their full performance report.",
        }),
      })
        .then((response) => {
          if (response.ok) {
